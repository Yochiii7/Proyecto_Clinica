      ...
      <table class="table align-middle">
        ...
        <tbody class="table-group-divider">
          <tr v-for="a in filteredAppointments" :key="a.id">
            ...
            <td>{{ a.time }}</td>
            <td>{{ formatDateTimeToDisplay(a.created_at) }}</td>
            <td>
          ...
          <tr v-if="filteredAppointments.length === 0">
            <td colspan="9" class="text-center">No hay citas en el rango seleccionado.</td>
          </tr>
        </tbody>
      </table>
        <h3>Reportes:</h3>
          ...
          <table class="table align-middle">
            ...
            <tbody>
              <tr v-for="r in reportResults" :key="`rep-${r.id}`">
                ...
                <td>{{ r.hora }}</td>
                <td>{{ formatDateTimeToDisplay(r.fecha_creacion || r.created_at) }}</td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <div class="form-row">
          <div class="form-group">
            ...
            <select class="form-control" id="patient" v-model="currentAppointment.cod_paciente" required>
              ...
              <option v-for="p in pacientes" :key="p.cod_paciente" :value="p.cod_paciente">{{ (p.nombre_paciente || p.nombre) + (p.apellido_paciente ? ' ' + p.apellido_paciente : '') }}</option>
            </select>
          </div>
          ...
          <div class="form-group">
            ...
            <select class="form-control" id="especialidad" v-model="currentAppointment.cod_especialidad" required @change="onEspecialidadChange">
              ...
              <option v-if="especialidadesLoading" disabled value="">Cargando especialidades...</option>
              <option v-else-if="!especialidadesLoading && especialidades.length === 0" disabled value="">No hay especialidades registradas</option>
              <option v-else v-for="e in especialidades" :key="e.cod_especialidad" :value="e.cod_especialidad">{{ e.nombre_especialidad }}</option>
            </select>
          </div>
          <div class="form-group">
            ...
            <select class="form-control" id="doctor" v-model="currentAppointment.cod_doctor" :disabled="!currentAppointment.cod_especialidad || doctores.length === 0" required>
              ...
              <option v-for="d in doctores" :key="d.cod_doctor" :value="d.cod_doctor">{{ (d.nombre_doctor || d.nombre) + (d.apellido_doctor ? ' ' + d.apellido_doctor : '') }}</option>
            </select>
          </div>
        </div>
...
const reportFilterBy = ref('fecha')
  ...
  try {
    ...
    if (Array.isArray(res.data)) {
      ...
      appointments.value = res.data.map(c => ({
        ...
        especialidad: c.especialidad || c.nombre_especialidad || c.nombre || '',
        status: c.estado || c.status || '',
        date: c.fecha || c.date || '',
        time: c.hora || c.time || '',
        created_at: c.fecha_creacion || c.fechaCreacion || c.created_at || '',
        cod_paciente: c.cod_paciente || c.codPaciente || null,
        cod_doctor: c.cod_doctor || c.codDoctor || null,
        cod_especialidad: String(c.cod_especialidad || c.codEspecialidad || ''),
        notes: c.notes || ''
      }))
    } else {
    ...
    // Provide a more detailed error to help debugging (status code or message)
    if (err && err.response) {
      const status = err.response.status
      const statusText = err.response.statusText || ''
      const serverMsg = err.response.data && err.response.data.message ? ` - ${err.response.data.message}` : ''
      error.value = `No se pudo conectar al servidor de citas. (${status} ${statusText}${serverMsg})`
    } else if (err && err.message) {
      error.value = `No se pudo conectar al servidor de citas. (${err.message})`
    } else {
...
async function fetchEspecialidades() {
  ...
  try {
    ...
    // Normaliza los campos para asegurar que cada especialidad tenga cod_especialidad (string) y nombre_especialidad
    if (Array.isArray(res.data)) {
      console.debug('fetchEspecialidades -> raw:', res.data)
      especialidades.value = res.data.map(e => ({
        cod_especialidad: String(e.cod_especialidad || e.id || ''),
        nombre_especialidad: e.nombre_especialidad || e.nombre || ''
      }))
      console.debug('fetchEspecialidades -> normalized:', especialidades.value)
    } else {
...
async function editarCita(cita) {
  ...
  // Normalizar ids a string para que coincidan con los valores del select
  if (currentAppointment.value.cod_especialidad) currentAppointment.value.cod_especialidad = String(currentAppointment.value.cod_especialidad)
  if (currentAppointment.value.cod_doctor) currentAppointment.value.cod_doctor = String(currentAppointment.value.cod_doctor)
  if (currentAppointment.value.cod_paciente) currentAppointment.value.cod_paciente = String(currentAppointment.value.cod_paciente)
  editando.value = true
  showModal.value = true
  ...
  if (currentAppointment.value.cod_doctor) {
    const found = doctores.value.find(d => String(d.cod_doctor) === String(currentAppointment.value.cod_doctor))
  ...
  if (editando.value) {
    ...
    axios.put(`${API_BASE}/api/citas/${id}`, payload)
      .then(() => {
        ...
        try { window.dispatchEvent(new Event('appointments-changed')) } catch { /* noop */ }
      })
      .catch(err => {
...
function formatDateTimeToDisplay(value) {
  ...
  // If format 'YYYY-MM-DD HH:MM:SS' -> convert to ISO
