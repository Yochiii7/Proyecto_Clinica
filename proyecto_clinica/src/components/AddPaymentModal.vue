      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <div class="mb-2">
            ...
            <select v-model="form.cod_paciente" class="form-control" required>
              <option value="">Seleccione un paciente</option>
              <option v-for="paciente in pacientes" :key="paciente.value" :value="paciente.value">
                {{ paciente.label }}
              </option>
            </select>
          </div>
          <div class="mb-2">
            <label>Médico</label>
            <select v-model="form.cod_medico" class="form-control" required>
              <option value="">Seleccione un médico</option>
              <option v-for="medico in medicos" :key="medico.value" :value="medico.value">
                {{ medico.label }}
              </option>
            </select>
          </div>
    ...
    async cargarListas() {
      try {
        const [pacientesRes, medicosRes] = await Promise.all([
          fetch('http://localhost:3000/api/listas/pacientes'),
          fetch('http://localhost:3000/api/listas/doctores')
        ]);
        
        this.pacientes = await pacientesRes.json();
        this.medicos = await medicosRes.json();
      } catch (error) {
        console.error('Error al cargar listas:', error);
      }
    },
    save() {
      if (!this.form.cod_paciente || !this.form.cod_medico || !this.form.concepto || this.form.monto <= 0) {
